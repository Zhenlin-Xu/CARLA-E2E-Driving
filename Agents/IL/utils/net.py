...
class MultiModActor(nn.Module):
    ...
    def __init__(self):
        ...
        out_model = []
        out_model += [nn.Linear(in_features=in_channels[4]+9, out_features=in_channels[5])]
        out_model += [nn.ReLU()]
    ...
    def forward(self, obs, state, info=None):    
        ...
        x = self.conv(x).flatten(1) # [1, 64, 25, 25] -> [1, 120000]
        x = self.mlp(x)
        # # concatenate the state and action
        x = torch.concat((x, val), dim=1)
        # # preprocess all the state and action 
...
#     inp = torch.ones(1,6*100*200+9)
#     print(inp.shape)

#     net = MultiModActor().to("cuda")
